...
func regenerateHandoff() {
	...
	// BOOT.md FIRST, AND THE OLD AUTO SECTION GOES - 2026-09-12 (brain two v0;
	// Sleven's go, Architecture's shape: both halves in ONE change).
	//
	// The "CURRENT STATE (auto)" section printed ccpp health figures computed on
	// a 4-ship test set - "4 ships in total" against 253 front-page cards - and
	// CLAUDE.md already told every reader not to trust it. It is replaced by one
	// line pointing at BOOT.md, so there is one current page with one writer.
	// buildAutoBlock is left in place, uncalled, for the record; the ccpp scan
	// itself still runs every cycle in rescanAndScore().
	bootPath := filepath.Join(projectRoot, "BOOT.md")
	var stateBlock string
	if err := writeBootLocked(projectRoot, bootPath, time.Now()); err != nil { // the beat writes it too - see ticker.go
		logMsg("could not write %s: %v", bootPath, err)
		stateBlock = fmt.Sprintf("**BOOT.md could NOT be written this cycle** (%v). "+
			"Do not trust an older copy's generated time; read the deep files instead.", err)
	} else {
		stateBlock = fmt.Sprintf("**The current state is [`BOOT.md`](BOOT.md), regenerated by this watcher at %s. "+
			"Read it first.** This section used to print auto-computed health figures from a 4-ship test set; "+
			"they were wrong and were removed on 2026-09-12.", time.Now().Format("2006-01-02 15:04:05"))
	}
