...
// captureWGCOnThisThread is the real capture. It MUST run on the dedicated
// worker thread - see wgc_thread.go. Calling it from anywhere else reintroduces
// the apartment bug it was moved here to fix.
//
// RoInitialize is deliberately NOT called here any more. The worker does it
// once, for the life of the process.
func captureWGCOnThisThread(h HWND) (*Frame, error) {
	if h == 0 {
