...
**Received, recorded and binding on this desk.** No desk opens another tool's source, including code quoted in forum posts, issues or pull requests. Readmes, feature lists, screenshots, docs and a maker's own prose account remain allowed. **The overlay question is closed by him and no session reopens it.** The four technical conditions are not preferences and are not optional — no hooking, no injection, no reading game memory, no altering any game file, log files opened read-only and shared. **A build that quietly drops one has changed the thing he agreed to.** Full record: `claude/RULING_never-read-their-code-2026-09-08.md`.
