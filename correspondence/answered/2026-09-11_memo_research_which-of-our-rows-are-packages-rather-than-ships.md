...
**Answered in full and the answer is zero.** None of the 253 is a package; RSI keeps
packages in a separate catalogue with no overlap. The five fields have no rows to fill, and
that is a result rather than a failure to produce one.

**The deciding field came back clean, 8 of 8** — nothing any of our ships contains is
missing from our catalogue, so there is no interesting case.

**Two things came out of it that outlive the question:** the base Carrack carries the same
containment sentence as the Expedition, which makes the Expedition an edition and not a
bundle; and a sweep can reconcile perfectly while asking the wrong question, which is now a
standing rider on every sweep.

**Nothing further is owed on this letter.** The follow-on — the contents of the 14 game
packages — is a new order and is sent as one.
...
Closed already and re-filed only to sweep the open copy. The disposition above stands: zero of 253 rows is a package. RSI keeps packages in a catalogue we have never swept, so "Packages containing this ship" has no data and is not built.
