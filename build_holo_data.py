...
# THE SECOND OVERLAY: CIG'S OWN COORDINATES, out of the ship geometry in
# Data.p4k. Generated by build_hardpoint_overlay.py from the transforms
# decoded in docs/FINDING_the-coordinates-are-in-the-client-2026-08-27.md.
#
# WHY A SECOND FILE RATHER THAN MERGING INTO THE FIRST. The file above is
# somebody's deliberate correction, made by hand, one hull at a time. This one
# is generated in bulk and can be regenerated wrongly. Keeping them apart means
# a bad generation is reverted by deleting one path, and it means a human
# decision is never silently outvoted by a script - which is why the ORDER
# below matters: the client positions go on first, the hand-made overlay goes
# on second, and where they touch the same port the hand-made one wins.
ALIGN_CLIENT = os.path.join(HERE, "data-layer", "derived",
...
def merge_join(fleet):
    ...
    merged, collisions, duplicates, alias = 0, [], [], {}
    for stem, rec in sorted(joined.items()):
        ...
        if key in fleet:
            # A COLLISION IS TWO DIFFERENT QUESTIONS AND THIS USED TO ASK ONLY
            # THE SECOND.
            #
            # "One of the two is wrong about which hull it is" is the dangerous
            # case and it still exits. But it is not the only way a key can
            # repeat: the SAME hull can arrive twice, once from the placement
            # pass and once from the recovery join, and refusing that is how
            # this generator stopped emitting on 17 August and left the holo
            # page on a ten-day-old build.
            #
            # The discriminator is the model file. Two records naming the same
            # .glb are the same hull; there is no question of a Gladius wearing
            # somebody else's hardpoints, because there is only one hull in
            # play. Two records naming DIFFERENT models is the real ambiguity
            # and still refuses.
            #
            # WHICH ONE WINS, AND WHY (Sleven, 2026-08-27, on
            # FINDING_the-holo-collision-is-one-hull-twice): the PLACED record.
            # It carries per-hardpoint provenance the recovered one has as null
            # - placed_from, aimed_at, depth - and placed_from is what the
            # disclosure work needs to tell a marker derived from a mount name
            # apart from one that is CIG's own transform. The recovered record's
            # extra fields are bookkeeping about the recovery, not claims about
            # the hull.
            #
            # This is a SKIP, not an overwrite. The single-writer rule on
            # hardpoints_fleet.json is untouched.
            placed = fleet.get(key) or {}
            if rec.get("model") and rec.get("model") == placed.get("model"):
                duplicates.append([stem, key, rec.get("model")])
                continue
            collisions.append([stem, key, rec.get("model"), placed.get("model")])
            continue
        fleet[key] = rec
        alias[stem] = key
        merged += 1

    for stem, key, model in duplicates:
        say("  duplicate: %s is the same hull as %r (%s) - keeping the placed "
            "record" % (stem, key, model))

    if collisions:
        for stem, key, rmodel, pmodel in collisions:
            say("  COLLISION: %s would overwrite %r - refused "
                "(recovered model %r vs placed model %r)"
                % (stem, key, rmodel, pmodel))
        sys.exit("%d recovered ship(s) name a DIFFERENT model than the ship they "
                 "would overwrite. Refusing to emit: one of the two is wrong "
                 "about which hull it is, and picking silently is how a Gladius "
                 "ends up wearing somebody else's hardpoints." % len(collisions))
    return fleet, {"merged": merged, "alias": alias,
                   "duplicates_skipped": [d[0] for d in duplicates]}


def _apply_one(fleet, alias, path, label):
    """Apply one overlay file. Returns the number of ports moved.
    ...
    if not os.path.exists(path):
        return 0, "no %s overlay present" % label
    ...
    for key, ports in overlay.items():
        ...
        for port, pos in ports.items():
            ...
            if h is None:
                ...
                continue
            h["unit"] = pos["unit"]
            # pos_model is OPTIONAL in the client overlay. A hull whose own
            # existing records gave no unit/pos_model pair to scale back out
            # emits none rather than a guessed one, and the record keeps the
            # pos_model it already had. `unit` is what the page renders.
            if "pos_model" in pos:
    ...
    if unknown:
        for u in unknown[:20]:
            say("  %s OVERLAY names something that is not here: %s" % (label, u))
        sys.exit("%d %s overlay entr(ies) matched nothing. Refusing to emit: an "
                 "overlay that silently matches nothing reports a fix it did not "
                 "make." % (len(unknown), label))
    return moved, None


def apply_alignment(fleet, alias):
    """Both overlays, in the order that makes a human decision win.

    CLIENT FIRST, HAND-MADE SECOND. The client overlay carries CIG's own
    per-hardpoint transforms for the hulls it covers; the hand-made overlay is
    a person's correction on a specific hull and must not be reverted by a bulk
    regeneration. Applying it last is what guarantees that, without either file
    needing to know the other exists.
    """
    note = {}
    n, why = _apply_one(fleet, alias, ALIGN_CLIENT, "client")
    note["client_moved"] = n
    if why:
        note["client_note"] = why
    n, why = _apply_one(fleet, alias, ALIGN, "manual")
    note["moved"] = n
    if why:
        note["note"] = why
    return fleet, note
