...
# R7/H5: the checksum of the data file, published beside it so a visitor can
# check every number against the file the page actually read. It is a SECOND
# file for one unavoidable reason - a file cannot contain its own sha256 - and
# it is generated, never typed, so the two cannot disagree.
SUM_OUT = os.path.join(HERE, "testing", "_src", "find_checksum.gen.js")
...
CHECKSUM_HEADER = """/* GENERATED by build_find_data.py - do not hand edit.
...
def render_checksum(filename, digest, size):
    """The published checksum. Deterministic - derived only from the bytes."""
    return (CHECKSUM_HEADER
            + "const FIND_CHECKSUM=" + _dump({
                "file": filename, "sha256": digest, "bytes": size,
                "algorithm": "sha256",
            }) + ";\n")
...
def main():
    ...
    ap.add_argument("--sum-out", default=SUM_OUT,
                    help="where the published checksum goes (H5).")
    ...
    if args.check:
        ...
        # The published checksum is checked too. A correct data file beside a
        # checksum describing the PREVIOUS one is worse than no checksum at
        # all: it invites a visitor to conclude the file was tampered with.
        want_sum = render_checksum(os.path.basename(args.out),
                                   hashlib.sha256(raw).hexdigest(), len(raw))
        if not os.path.exists(args.sum_out):
            sys.exit("STALE: %s does not exist. Run build_find_data.py."
                     % os.path.relpath(args.sum_out, HERE))
        with open(args.sum_out, "r", encoding="utf-8", newline="") as fh:
            if fh.read() != want_sum:
                sys.exit("STALE: %s does not describe the current data file. "
                         "Re-run build_find_data.py."
                         % os.path.relpath(args.sum_out, HERE))
        print("up to date: %s and %s both match the database"
              % (os.path.relpath(args.out, HERE),
                 os.path.relpath(args.sum_out, HERE)))
        return 0

    digest = hashlib.sha256(raw).hexdigest()
    sum_text = render_checksum(os.path.basename(args.out), digest, len(raw))

    if args.check:
        pass  # unreachable - --check returned above. Kept out of the way.

    with open(args.out, "w", encoding="utf-8", newline="") as fh:
        fh.write(text_out)
    with open(args.sum_out, "w", encoding="utf-8", newline="") as fh:
        fh.write(sum_text)
    print("written: %s" % os.path.relpath(args.out, HERE))
    print("written: %s" % os.path.relpath(args.sum_out, HERE))
