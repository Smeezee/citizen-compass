...
**Authoritative as of 2026-08-09.** The sections immediately below are the newest and win over anything later in this file.
...
## THE BUILD PIPELINE, READ AND WRITTEN DOWN — 2026-08-09

Read off the project machine, not inferred: `build_deploy.py` (21,290 bytes),
`inject_engine.py`, `check_deploy_clean.py`. **This corrects an over-broad claim made
earlier in this document and repeated by C3.**

`python testing/_src/build_deploy.py` does four things in order: builds `_deploy/index.html`
from `releases/latest.html` + `_layer.src.html` with substitutions and the password gate;
runs `inject_engine.py`; copies `PAGES` **verbatim**; runs the deploy guard.

**`keybinds.src.html` is copied verbatim — it is NOT generated.** Edits to it land exactly as
written. The "patching the source layer can silently do nothing" warning applies to
`_layer.src.html`, which becomes `index.html`, not to the keybind page.

**The one exception, and it is the important one:** `inject_engine.py` overwrites the block
of `keybinds.src.html` between `DEVICE PANEL rev 2` and `if(dev!=="KBM") renderDevice(); });`
with the contents of `device_engine.js`, on every build, in silence, because one-writer is
its whole job. **Device-panel work goes in `device_engine.js` or it is deleted.**

Three things that decide how any page work is done:

- **Adding a page is one line in `PAGES`.** The deploy guard's allowed set is computed from
  `PAGES`, so it follows automatically.
- **A new top-level directory under `_deploy/` fails the guard.** Only `images` and `models`
  are allowed; anything else needs a deliberate edit to `DEFAULT_ALLOWED_DIRS`.
- **`_layer.src.html` is a SECOND host for the device engine**, and the index page carries
  its own keybind overlay (`kb_overlay.inc.html`, 40 KB). Device-panel changes reach both
  automatically; **keybind-UI changes made only in `keybinds.src.html` do not**, and nothing
  in the build will notice the two drifting. Not previously flagged anywhere.

Detail and what it means for the two outstanding jobs:
`claude/WORKORDER_builder-ui-and-viewer-2026-08-09.md`.
...
1. **Rotate the UEX token** — exposed in a screenshot, still unrotated.
2. **Change the PostgreSQL password** — also exposed. The more serious of the two.
3. **Roll the Cloudflare token** — reached chat twice, sits in transcripts.
4. **Settle the description-rights question** with CIG.
5. **Offsite backup** — the backup itself is now fixed and verified (2026-08-07: repointed to the My Book on D:, E: as second mirror, per-file verification — 951/951 sc-ships and 58,257/58,257 external-sources on both, database backed up and restore-verified at 232 ships in = 232 out). Genuinely offsite storage is still open.
6. **Three item-taxonomy rulings** (2026-08-07) — see `claude/workorder-catalog-01-fullset-junkdrawer-manufacturer-commodities.md`.
7. **Five mission-template DebugName tokens need a game-knowledge call** (mislabeled "keybind tokens" in earlier versions of this doc — corrected 2026-08-07. These are NOT keybind-related; they're tokens inside Star Citizen's internal mission names, flagged in `claude/FINDING_missions-and-locations-full-breakdown.md` §3): `Group`, `Inhabited`, `NoNonHostiles`, `CrossStanton`, `DC`. The call: should each be treated as cosmetic flavor (strip it, mission counts as the same template as one without it) or as a real mechanical difference (keep it, count as a distinct template)? Affects the Family B mission-template estimate by roughly 350 templates (700 vs 1,051). See the finding doc's §3 for the actual DebugName example and full context.
8. ~~Open CIG's Fan Kit and check what its 3D models actually contain~~ **CLOSED 2026-08-08.** It was already on the machine. Read in full — see the Fan Kit section at the top of this document and `claude/FINDING_fankit-inventory-2026-08-08.md`. Answer: 14 ships, bare geometry, doesn't resolve the hardpoint/skin problem, but the logos and the exact image-marking requirements are real and usable.
9. **Review the flagged rows** (2026-08-07): the 5 keybind descriptions the wire-in flagged, the HELP drawer wording, and the 4 medium-confidence entries in `keybind_troubleshooting.json`.
