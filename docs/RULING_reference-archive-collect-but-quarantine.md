...
## 2. Why a policy alone will not hold — this project's own record

**This is not hypothetical. The exact failure has already happened here:**

- **`testing/_deploy/` publishes whatever is in it.** A `.wrangler/cache` folder went out that
  way — account ID and name reached the open internet. Nobody intended it.
- **The password gate does not cover static assets.** On 2026-08-07 `models/100i.glb` was
  fetched directly from the "private preview" and returned binary. The HTML asks for a
  password; everything else does not.
- The project has now logged **five** separate silent-success failures where something was
  published, deleted or skipped while the run reported success.

**So "we will hold it but not publish it" is a promise this project's own pipeline has already
broken once by accident, on a folder nobody was even thinking about.** A reference corpus of
CIG's material sitting anywhere near the deploy tree will eventually go out the same way, and
the run will report success when it does.

**The fix is not more care. It is putting the corpus somewhere that cannot be published.**
...
**R5. Never in git, never in the database, never in a backup that feeds a deploy.** Git makes
removal a history rewrite rather than a delete: anything committed is in every clone and every
bundle forever, and "we removed it" becomes a lie the moment someone checks out an old commit.
**This is the single most expensive mistake available here and it is one `git add -A` away** —
note the standing caution against exactly that command already in `CURRENT-STATE.md`.
