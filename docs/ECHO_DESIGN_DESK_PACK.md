...
It is plain files. No accounts, no login, no server application. Data is generated at build
time and embedded in the pages.
...
- A **package** is several ships sold together; it gets its own section, "Packages
  containing this ship", listing exact contents with a link to each included ship's page,
  on every ship in the package.
- **No package price, ever.** We do not track it and will not imply we do. No "buy" or
  "available" labelling.
- Folded names stay searchable, and the result says why it matched: "Matched package:
  Carrack Expedition".
