# STATE ARCHIVE — everything CURRENT-STATE.md used to carry, through 2026-08-27

**THIS IS HISTORY. IT DOES NOT OVERRIDE ANYTHING.**

`docs/CURRENT-STATE.md` was 13,571 words ordered newest-first, opening with an
instruction that everything past a certain point was stale — without saying
where that point was. **The only safe way to read it was all of it, every
session, forever.** It was split on 2026-08-27.

**What that document now holds:** only what is true right now, short enough to
read in full. **What this file holds:** the whole of the old document, verbatim,
including the parts that were already history when it was split.

**READ THIS FILE ONLY TO ANSWER "WHY WAS IT DONE THAT WAY".** Never to find out
what is true. Where anything here disagrees with `CURRENT-STATE.md`, or with a
dated document in `docs/`, this file loses — it is not a source, it is a record
of what people believed on the day they wrote it.

**Known-stale examples, so nobody has to discover them:** the collector section
dated 08-07 says "None of it has run on Windows" — it built and passed 575
checks on 2026-08-27. The hardpoint-coverage numbers predate a night of work
that took the ship page from 64 hulls to 245 classes. Sections contradict each
other by design; that was the defect.

*Split by C1, 2026-08-27. Nothing was deleted — this is the original text.*
