...
const DATA_SRC = join(DEPLOY, "..", "_src", "find_data.gen.js");
...
const _fx = "var a = 1; /* fetch() in a block */\n// fetch() in a line\nvar b = 2;";
const _fxCode = _fx.replace(/\/\*[\s\S]*?\*\//g, "").replace(/^\s*\/\/.*$/gm, "");
record(/fetch\(\)/.test(_fx) && !/fetch/.test(_fxCode) && code.trim().length > 0,
  "the comment stripper works, proven on a local fixture, and the stripped "
  + "code is not empty - if this fails, every assertion below it is checking nothing");
...
/* THE GENERATOR NAMES ITSELF IN _src, AND DELIBERATELY NOT IN _deploy.
   This asserted the header on the DEPLOYED file until 2026-08-30. Q31 is why
   it moved: "GENERATED by build_find_data.py" names a build script and the
   repo layout on a public URL, and Sleven's instruction is that nothing on
   the public site hints at how it was built. The two requirements were in
   direct conflict and the instruction wins.
   The claim itself is still worth making - a generated file should say what
   made it - so it is made where the header still exists and where a reader
   who needs it will be. */
record(/GENERATED by build_find_data\.py/.test(readFileSync(DATA_SRC, "utf-8")),
  "and names the generator that wrote it - in _src, where it belongs");
record(!/GENERATED by/.test(dataJs),
  "and the DEPLOYED copy does not name it (Q31)");
record(!/generated at/i.test(dataJs.split("const FIND_SCHEMA")[0]),
  "and carries no generation timestamp (H6)");

console.log("\n--- H3: the banner is off, and what replaced it ---");
record(!/MOCKUP/.test(code),
  "the MOCKUP banner is gone from the CODE - removed only after "
  + "checks/_verify_find_deployed.mjs read real rows off the deployed URL");
record(!/prices and shops are invented/.test(html),
  "and so is its wording, comments included");
record(!/class="mock/.test(code),
  "and nothing on the page is still styled as a mockup");
record(/id="srcbadge"/.test(html),
  "a provenance badge sits where it was");
record(/price data not loaded/.test(html),
  "whose DEFAULT text, sitting in the markup, is the honest failure - so a "
  + "page whose data file did not load makes no claim at all");
