    ...
    for h in points:
        ...
        for i in (0, 1):
            ...
            if p[i] < box_min[i] - ctr[i] - m or p[i] > box_max[i] - ctr[i] + m:
                out += 1
                break
    # THE RULE: any exterior mount outside the hull refuses the whole hull.
    # This was briefly proportional ("more than half") and this check is what
    # refuted that - a transposed axis displaces only 10 of 59 mounts on the
    # Eclipse, so half is a threshold the defect walks straight through.
    return (n > 0 and out == 0), n, out
