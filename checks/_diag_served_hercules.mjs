 ...
 * finds the M2 Hercules, and counts the hardpoint dots actually in the DOM. */
...
const key = await page.evaluate(() => {
  ...
  const k = Object.keys(SHIPS).find(k => /M2 Hercules/i.test(SHIPS[k].n || ""));
  return k ? { key: k, name: SHIPS[k].n, cls: SHIPS[k].cls || null } : { err: "no Hercules" };
});
