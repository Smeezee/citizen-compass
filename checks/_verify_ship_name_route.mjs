...
console.log("\n--- and no post-render rewriter survives ---");
{
  ...
  record(!/decorate\s*\(\s*(document\b|[A-Za-z_$][\w$]*\s*\.\s*(body|documentElement)|\$\(\s*["'](?:ships?|shipTable|shipList|rows|tbl|index)["']\s*\))/i.test(code),
    "no decorate() is handed the container holding the name cells");
